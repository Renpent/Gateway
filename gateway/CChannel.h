...
public:
    ...
    /// 周期の最後に1回、全チャネルに対して呼ばれる。受信（pumpIn）と送信（pumpOut）の
    /// ループがどちらも終わったあとなので、状態を変えてもループを壊さない。
    ///
    /// 受信中に積んだものをここで処理する（CTRawChannel は自分のハンドラへ渡す）。
    /// **登録は要らない** — CGateway に足したチャネルはすべて呼ばれる。以前は CGateway に
    /// 処理を1つだけ登録する形だったが、ハンドラが増えたときに追記を忘れると、積まれたものが
    /// 処理されずにキューが伸び続けた。
