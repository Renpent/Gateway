...
bool Gateway::openAll(const std::string& peerHost) {
    ...
    for (auto& ch : channels_) {
        ...
        pollIndex_.push_back(poller_.add(ch->socket()));
    }
    opened_ = true;
    return true;
}
...
void Gateway::tick() {
    ...
    const int ready = poller_.wait(0);

    if (ready > 0) {
        for (std::size_t i = 0; i < channels_.size(); ++i) {
            if (poller_.readable(pollIndex_[i])) channels_[i]->pumpIn();
        }
    }

    // 送信は poll と無関係に毎周期。HLA 側から出てくるものは
    // ソケットの読み取り可否とは関係がない。
    for (auto& ch : channels_) ch->pumpOut();
}
...
void Gateway::run(unsigned hz, unsigned seconds) {
    ...
    for (auto& ch : channels_) ch->setLoopRate(hz);
    ...
    while (seconds == 0 || Clock::now() < deadline) {
        ...
        if (busy > loop_.maxTickMs) loop_.maxTickMs = busy;
        ...
        if (late > loop_.maxLateMs) loop_.maxLateMs = late;
        loop_.sumLateMs += late;
...
void Gateway::printSummary() const {
    ...
    const double avgLate = loop_.ticks ? loop_.sumLateMs / static_cast<double>(loop_.ticks) : 0.0;
    std::printf("\n周期        : %llu 周（超過 %llu 回）\n",
                static_cast<unsigned long long>(loop_.ticks),
                static_cast<unsigned long long>(loop_.overruns));
    std::printf("1周の処理   : 最大 %.3f ms\n", loop_.maxTickMs);
    std::printf("起床の遅れ  : 平均 %.3f ms / 最大 %.3f ms\n", avgLate, loop_.maxLateMs);
}
