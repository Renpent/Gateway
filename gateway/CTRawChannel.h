...
// 受け取って parse に渡し、成功したら CTMessageHandler へ、失敗したら数えて捨てる。
//
...
//       来ない前提で、来ても receive() が「何も来ていない」として読み捨てる（gateway/udp/CUdpSocket.h）。
...
public:
    ...
    [[nodiscard]] const udp::TUdpReceiveStats& getInStats() const noexcept override { return m_stats; }
    [[nodiscard]] const std::string& getLastError() const noexcept override {
        return m_sock.getLastError();
    }

private:
    gw::CTMessageHandler<T>* m_handler;             ///< アプリ側の受け口。借り物で、null なら数えて捨てる
    udp::CUdpSocket m_sock;                   ///< このメッセージ専用のソケット（受信のみ）
    std::vector<unsigned char> m_buf;   ///< 受信バッファ。UDP の最大長なので切り詰めは起きない
    udp::TUdpReceiveStats m_stats;            ///< datagrams/records = 受理、malformed = parse 失敗
};
