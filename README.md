...
| `STOP\r\n` | `"STOP"` | 行として。**末尾の改行を落とす** |
| （空）/ NUL だけ / 改行だけ | — | 形式違反。統計の「異常」に数える |
...
| 独自データの受信 | 見本の形式で確認。順番どおり届く、60,000 バイトも切り詰めない、形式違反と空データグラムを数える、ICD のクラスと同じ poll で動く、ポート重複を断る |
| コマンド文字列 | 実バイナリを `run none` で立ち上げ、別プロセスから送信（Windows / Linux）。長さぶん・改行付き・NUL 詰め・日本語の4件を処理、空と NUL だけの2件を異常に数える。受信中は積むだけで、処理が `tick()` の最後に回ることも確認 |
| 空データグラム・ポート重複 | 空データグラムが malformed に数えられ後続も同じ周期で読める。重複ポートは開く前に断る（Windows / Linux で挙動が逆なことも実測） |
